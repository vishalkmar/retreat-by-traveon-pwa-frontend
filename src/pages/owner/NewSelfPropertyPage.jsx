...
// Owner-driven Phase 1. We skip ownerName/ownerEmail/ownerPhone fields
// because the backend pulls them from the logged-in owner record.

const NewSelfPropertyPage = () => {
  ...
  const usePinned = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        let full = `${latitude}, ${longitude}`;
        try { full = await reverseGeocode(latitude, longitude); }
        catch { toast.error('Pinned location found, but address lookup failed'); }
        setForm((f) => ({ ...f, latitude, longitude, address: full, locationText: full }));
        toast.success('Current location pinned');
        setPinning(false);
      },
      (err) => { toast.error(err.message || 'Could not get location'); setPinning(false); },
  ...
  const submit = async (e) => {
    ...
    if (!form.latitude || !form.longitude) {
      return toast.error('Pin current location before continuing');
    }
    setSubmitting(true);
    try {
      const r = await api.post('/owner/self-properties', form);
      const property = r.data?.data?.property;
      toast.success('Phase 1 saved');
      navigate(`/owner/self/${property.id}/generate-id`);
    } catch (err) {
  ...
  return (
    <div className="app-shell">
      <TopBar title="Self-onboard · Phase 1" />
...
export default NewSelfPropertyPage;
