...
const SelfGenerateIdPage = () => {
  ...
  const onGenerate = async () => {
    setGenerating(true);
    try {
      const r = await api.post(`/owner/self-properties/${id}/generate-id`);
          ...
          <>
            ...
            <section className="mt-6 rounded-3xl border border-brand-100 bg-brand-50 p-6 text-center">
              ...
              <p className="mt-1 text-xs text-brand-800/80">
                {property?.propertyCode
                  ? 'This unique code stays with your property through review and contract.'
              ...
              {property?.propertyCode ? (
                ...
                <Button size="block" className="mt-5" onClick={onGenerate} loading={generating}>
            ...
            {property?.propertyCode && (
              <Button
                size="block"
                className="mt-6"
                onClick={() => navigate(`/owner/self/${id}/capture`)}
              >
...
export default SelfGenerateIdPage;
