...
const Phase4Page = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const r = await api.get(`/auditor/properties/${id}/phase4`);
      const b = r.data?.data;
      setBundle(b);
  ...
  const saveSection = async (sectionKey) => {
    ...
    try {
      await api.put(`/auditor/properties/${id}/phase4/${sectionKey}`, {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      await api.post(`/auditor/properties/${id}/phase4/submit`);
