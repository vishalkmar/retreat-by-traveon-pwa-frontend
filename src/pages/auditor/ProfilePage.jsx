      ...
      <main className="flex-1 overflow-y-auto p-4 pb-24">
        <section className="flex items-center gap-4 rounded-2xl border border-slate-100 bg-white p-4 shadow-card">
          {user?.profilePhotoUrl ? (
            ...
            <div className="grid h-16 w-16 place-items-center rounded-full bg-brand-100 text-xl font-semibold text-brand-800">
              {(user?.name || '?').slice(0, 1)}
            </div>
          )}
          <div className="min-w-0">
            ...
            <p className="mt-1 text-[11px] uppercase tracking-wider text-brand-700">Auditor</p>
          </div>
        </section>
